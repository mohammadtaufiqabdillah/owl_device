...
$stmt = $conn->prepare("SELECT * FROM device_category WHERE category_id = ? AND company_code = ?");
$stmt->bind_param("ii", $category_id, $company_code);
$stmt->execute();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            swal({
                ...
                text: 'Kategori yang dimaksud tidak tersedia.',
                icon: 'error',
                buttons: false,
                timer: 1200
            }).then(() => {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $conn->prepare("SELECT * FROM device_category WHERE company_code = ? AND description = ?");
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                swal({
                    ...
                    button: false,
                    timer: 1200,
                }).then(() => window.location.replace('./edit?category_id=$category_id'));
        });
        </script>";
        exit;
    }

    $update = $conn->prepare("UPDATE device_category SET description = ?, updated_by = ? WHERE category_id = ? AND company_code = ?");
    $update->bind_param("siii", $description, $user_id, $category_id, $company_code);
                ...
                <div class="page-inner">
                    ...
                    <div class="card card-round">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Edit Category Data</h4>
                        </div>
                        <form method="POST">
                            <div class="card-body">
