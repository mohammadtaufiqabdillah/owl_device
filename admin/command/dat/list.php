...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['soft_delete_id'])) {
    ...
    if ($stmt->affected_rows > 0) {
        echo json_encode([
            'status' => 'success',
            'message' => 'Command Dat berhasil dihapus (soft delete).'
        ]);
    } else {
        echo json_encode([
            'status' => 'error',
            'message' => 'Command Dat tidak ditemukan atau sudah dihapus.'
        ]);
    }
    $stmt->close();
    exit;
}
$stmt = $conn->prepare("SELECT * FROM device_command WHERE company_code = ? AND command_type = 'dat' AND is_deleted = 0 ORDER BY command_code DESC");
...
<body>
            ...
            <div class="container">
                <div class="page-inner">
                    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
                        <div>
                            <h3 class="fw-bold mb-3">List Command Dat</h3>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card card-round">
                                <div class="card-header align-center">
                                    <a href="admin/command/dat/create">
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                <a href="admin/command/dat/edit?command_id=<?php echo $row['command_id']; ?>"
                                                                    class="btn btn-info btn-sm">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                                <a href="admin/command/dat/detail/list?command_id=<?php echo $row['command_id']; ?>"
    ...
    <script>
        $(document).ready(function () {
            $('#tableCommandDat').on('click', '.soft-delete-btn', function (e) {
                e.preventDefault();
                var userId = $(this).data('id');

                swal({
                    title: "Hapus Command Dat?",
                    text: "Command Dat akan dihapus (soft delete) dan disembunyikan dari list jadi is_deleted = 1 apabila ingin tampil is_deleted = 0.",
                    icon: "warning",
                    buttons: {
                        cancel: {
                            text: "Batal",
                        ...
                        success: function (res) {
                            if (res.status === "success") {
                                swal({
                                    icon: "success",
                                    title: "Berhasil!",
                                    text: res.message || "Command Dat berhasil dihapus.",
                                    buttons: false,
                                    timer: 1200
                                }).then(function () {
                                    window.location.replace("admin/command/dat/list");
                                });
                            } else {
                                swal({
                                    icon: "error",
                                    title: "Gagal!",
                                    text: res.message || "Gagal menghapus Command Dat."
                                });
                            }
                        },
                        error: function (xhr) {
            ...
            var table = $('#tableCommandDat').DataTable({
                pageLength: 10,
                lengthMenu: [
                    [10, 25, 50, -1],
                    ['10', '25', '50', 'All']
