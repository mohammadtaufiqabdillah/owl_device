...
if (!$company_code) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            swal({
                title: 'Tidak Bisa Membuat Command Dat!',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $command_name = trim($_POST['nameCommandDat']);
    ...
    $stmt->bind_param("iiisisss", $company_code, $nextKode, $user_id, $command_name, $is_main, $custom_class, $description, $command_type);
    ...
    if ($stmt->execute()) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                swal({
                    title: 'Berhasil!',
                    text: 'Error Command Dat berhasil ditambahkan.',
                    icon: 'success',
                    button: false,
                    timer: 1200,
                }).then(() => window.location = './admin/command/dat/list');
            });
        </script>";
    } else {
        echo "<script src='../../../assets/js/plugin/sweetalert/sweetalert.min.js'></script>
...
<head>
    ...
    <title>Create Command Dat</title>
            ...
            <div class="container">
                <div class="page-inner">
                    <div class="d-flex align-items-center justify-content-between pt-2 pb-4">
                        <h3 class="fw-bold mb-3">Create Command Dat</h3>
                    </div>
                    <div class="card card-round">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Add Command Dat Data</h4>
                        </div>
                        <form method="POST">
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="nameCommandDat">Command Name</label>
                                    <input type="text" class="form-control form-control-border border-width-2"
                                        id="nameCommandDat" name="nameCommandDat" placeholder="Enter command name"
                                        required>
                                </div>
                                <div class="form-group">
                            ...
                            <div class="card-footer d-flex justify-content-end gap-3">
                                ...
                                <button type="button" class="btn btn-secondary"
                                    onclick="window.location.href='admin/command/dat/list'">Cancel</button>
                            </div>
                        </form>
                    </div>
                </div>
