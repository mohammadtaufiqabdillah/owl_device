...
if ($command_id <= 0 || $command_detail_id <= 0) {
    echo "<script>alert('Parameter tidak valid!'); window.location='../../../../admin/command/dat/list';</script>";
    exit;
}

$qDat = $conn->prepare("SELECT * FROM device_command WHERE command_id = ? AND company_code = ?");
$qDat->bind_param("ii", $command_id, $company_code);
$qDat->execute();
...
$qdDat = $conn->prepare("SELECT * FROM device_command_detail WHERE command_detail_id = ?");
$qdDat->bind_param("i", $command_detail_id);
$qdDat->execute();
$qdResult = $qdDat->get_result();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            swal({
                ...
                window.location = '../list?command_id=$command_id';
            });
        });
    </script>";
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE device_command_detail 
                            SET data_name = ?, data_type = ?, data_len = ?, data_default = ?, updated_by = ? 
                            WHERE command_detail_id = ?");
    ...
    if ($stmt->execute()) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                window.location = './admin/command/dat/detail/edit?command_id=$command_id&command_detail_id=$command_detail_id';
                });
            });
        </script>";
        exit;
...
<head>
    ...
    <title>Edit Command Dat Detail</title>
                ...
                <div class="page-inner">
                    <div class="pt-2 pb-4">
                        <h3 class="fw-bold mb-3">
                            edit :: dat :: <?= htmlspecialchars($datName) ?> ::
                            <?= htmlspecialchars($ddat['data_name']) ?>
                        </h3>
                    </div>

                    <div class="card card-round">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Edit Command Dat Detail</h4>
                        </div>
                        <form method="POST">
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="data_name" name="data_name"
                                        value="<?= htmlspecialchars($ddat['data_name']) ?>" required>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="data_type" id="data_type" class="form-control" required>
                                        ...
                                        <option value="string" <?= $ddat['data_type'] === 'string' ? 'selected' : '' ?>>
                                            String</option>
                                        <option value="object" <?= $ddat['data_type'] === 'object' ? 'selected' : '' ?>>
                                            Object</option>
                                        <option value="bool" <?= $ddat['data_type'] === 'bool' ? 'selected' : '' ?>>Boolean
                                        </option>
                                        <?php
                                        $types = ['uint8_t', 'uint16_t', 'uint32_t', 'uint64_t', 'int8_t', 'int16_t', 'int32_t', 'int64_t', 'float'];
                                        foreach ($types as $t) {
                                            $selected = ($t == $ddat['data_type']) ? 'selected' : '';
                            ...
                            <div class="card-footer d-flex justify-content-end gap-3">
                                ...
                                <button type="button" class="btn btn-secondary"
                                    onclick="window.location.href='admin/command/dat/detail/list?command_id=<?= $command_id ?>'">
                                    Cancel
                                </button>
                            </div>
                        </form>
