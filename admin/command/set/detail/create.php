...
if (!$command_id) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            swal({
                title: 'Tidak Bisa Membuat Command Detail Set!',
                text: 'Command ID tidak ditemukan.',
                icon: 'warning',
                button: false,
                timer: 1200,
            }).then(() => {
                window.location = '../../set/detail/list';
            });
        });
    </script>";
    exit;
...
$setName = '';
$qSet = $conn->prepare("SELECT command_name FROM device_command WHERE command_id = ?");
$qSet->bind_param("i", $command_id);
$qSet->execute();
$qResult = $qSet->get_result();
if ($qResult->num_rows > 0) {
    $exe = $qResult->fetch_assoc();
    $setName = $exe['command_name'];
}
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    swal({
                        ...
                        window.location = './admin/command/set/detail/create?command_id=$command_id';
                    });
                });
            </script>";
            exit;
...
<head>
    ...
    <title>Create Command Set Detail</title>
            ...
            <div class="container">
                <div class="page-inner">
                    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
                        <div>
                            <h3 class="fw-bold mb-3">add :: set :: <?= htmlspecialchars($setName) ?></h3>
                        </div>
                    </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="card-footer d-flex justify-content-end gap-3">
                                ...
                                <button type="button" class="btn btn-secondary"
                                    onclick="window.location.href='admin/command/set/detail/list?command_id=<?= $command_id ?>'">Cancel</button>
                            </div>
                        </form>
                    </div>
                </div>
