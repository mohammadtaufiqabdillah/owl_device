...
if ($command_id <= 0 || $command_detail_id <= 0) {
    echo "<script>alert('Parameter tidak valid!'); window.location='../../../../admin/command/res/list';</script>";
    exit;
}

$qRes = $conn->prepare("SELECT * FROM device_command WHERE command_id = ? AND company_code = ?");
$qRes->bind_param("ii", $command_id, $company_code);
$qRes->execute();
...
$resName = $res['command_name'] ?? '';

$qdRes = $conn->prepare("SELECT * FROM device_command_detail WHERE command_detail_id = ?");
$qdRes->bind_param("i", $command_detail_id);
$qdRes->execute();
$qdResult = $qdRes->get_result();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            swal({
                ...
                window.location = 'admin/command/res/detail/list?command_id=$command_id';
            });
        });
    </script>";
    exit;
    ...
    if ($stmt->execute()) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                swal({
                    ...
                    button: false,
                    timer: 1200
                }).then(() => window.location = './admin/command/res/detail/list?command_id=$command_id');
            });
        </script>";
    } else {
        echo "<script src='../../../../assets/js/plugin/sweetalert/sweetalert.min.js'></script>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                window.location = './admin/command/res/detail/edit?command_id=$command_id&command_detail_id=$command_detail_id';
                });
            });
        </script>";
        exit;
...
<head>
    ...
    <title>Edit Command Res Detail</title>
                ...
                <div class="page-inner">
                    <div class="pt-2 pb-4">
                        <h3 class="fw-bold mb-3">
                            edit :: res :: <?= htmlspecialchars($resName) ?> ::
                            <?= htmlspecialchars($dres['data_name']) ?>
                        </h3>
                    </div>

                    <div class="card card-round">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Edit Command Res Detail</h4>
                        </div>
                        <form method="POST">
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="data_name" name="data_name"
                                        value="<?= htmlspecialchars($dres['data_name']) ?>" required>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="data_type" id="data_type" class="form-control" required>
                                        ...
                                        <option value="string" <?= $dres['data_type'] === 'string' ? 'selected' : '' ?>>
                                            String</option>
                                        <option value="object" <?= $dres['data_type'] === 'object' ? 'selected' : '' ?>>
                                            Object</option>
                                        <option value="bool" <?= $dres['data_type'] === 'bool' ? 'selected' : '' ?>>Boolean
                                        </option>
                                        <?php
                                        $types = ['uint8_t', 'uint16_t', 'uint32_t', 'uint64_t', 'int8_t', 'int16_t', 'int32_t', 'int64_t', 'float'];
                                        foreach ($types as $t) {
                                            $selected = ($t == $dres['data_type']) ? 'selected' : '';
                            ...
                            <div class="card-footer d-flex justify-content-end gap-3">
                                ...
                                <button type="button" class="btn btn-secondary"
                                    onclick="window.location.href='admin/command/res/detail/list?command_id=<?= $command_id ?>'">
                                    Cancel
                                </button>
                            </div>
                        </form>
