...
if (!$command_id) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            swal({
                title: 'Tidak Bisa Membuat Command Detail Res!',
                text: 'Command ID tidak ditemukan.',
                icon: 'warning',
                button: false,
                timer: 1200,
            }).then(() => {
                window.location = '../../res/detail/list';
            });
        });
    </script>";
    exit;
...
$resName = '';
$qRes = $conn->prepare("SELECT command_name FROM device_command WHERE command_id = ?");
$qRes->bind_param("i", $command_id);
$qRes->execute();
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    swal({
                        ...
                        window.location = './admin/command/res/detail/create?command_id=$command_id';
                    });
                });
            </script>";
            exit;
...
<head>
    ...
    <title>Create Command Res Detail</title>
            ...
            <div class="container">
                <div class="page-inner">
                    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
                        <div>
                            <h3 class="fw-bold mb-3">add :: res :: <?= htmlspecialchars($resName) ?></h3>
                        </div>
                    </div>
                        ...
                        <form method="POST">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control form-control-border border-width-2"
                                        id="data_default" name="data_default"
                                        placeholder="Enter command default value..." required>
                                </div>
                            </div>
                            ...
                            <div class="card-footer d-flex justify-content-end gap-3">
                                ...
                                <button type="button" class="btn btn-secondary"
                                    onclick="window.location.href='admin/command/res/detail/list?command_id=<?= $command_id ?>'">Cancel</button>
                            </div>
                        </form>
                    </div>
                </div>
