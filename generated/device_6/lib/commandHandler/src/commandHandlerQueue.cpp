...
bool CommandHandler::loadStruct(stb_data_air_struct &structData, bool internal, bool loadJson, JsonDocument *docResult) {
 JsonDocument doc;
 if (!loadJson) {
 bool hasValue = false;
 if (internal && loadSettingsInternal("stb_data_air", doc)) {
 return true;
 }
 if (!internal && loadSettings("stb_data_air", doc)) {
 return true;
 }
 if (!hasValue) {
 return true;
 ...
 if (loadJson && docResult != nullptr) {
 (*docResult)["command"] = "stb_data_air";
 return false;
 }
